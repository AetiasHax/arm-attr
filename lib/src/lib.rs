    ...
    pub fn into_public_attributes(self) -> Result<PublicAttributes<'a>, PublicAttrsError> {
        ...
        for tag in tags {
            ...
            match tag {
                ...
                Tag::AsimdArch(value) => attrs.asimd_arch = Some(AsimdArch::from(value)),
                ...
                Tag::AbiOptGoals(value) => attrs.abi_opt_goals = Some(AbiOptGoals::from(value)),
                Tag::AbiFpOptGoals(value) => attrs.abi_fp_opt_goals = Some(AbiFpOptGoals::from(value)),
                Tag::Compat { flag, vendor_name } => attrs.compat = Some(Compat::new(flag, vendor_name)),
                Tag::CpuUnalignedAccess(value) => attrs.cpu_unaligned_access = Some(CpuUnalignedAccess::from(value)),
                Tag::FpHpExt(value) => attrs.fp_hp_ext = Some(FpHpExt::from(value)),
                Tag::AbiFp16BitFormat(value) => attrs.abi_fp_16bit_format = Some(AbiFp16BitFormat::from(value)),
                Tag::MpExtUse(value) => attrs.mp_ext_use = Some(MpExtUse::from(value)),
                Tag::DivUse(value) => attrs.div_use = Some(DivUse::from(value)),
                Tag::DspExt(value) => attrs.dsp_ext = Some(DspExt::from(value)),
                ...
                Tag::VirtualUse(value) => attrs.virtual_use = Some(VirtualUse::from(value)),
